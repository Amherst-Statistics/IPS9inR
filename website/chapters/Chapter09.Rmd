...
To create the summary table in  Example 9.1 we will have to compute some intermediate steps. 

To start, we first get the total counts for each sex. 

```{r Instag2}
# XX should we be using tidyverse?
Combined_Sex <- Instag %>% 
  group_by(Sex) %>%
  summarize(n = n()) 
Combined_Sex
...
This will be the first column of our table. Then we get a dataframe with only the counts of those who have a value of "Yes" for User. This will be the second column.
...
And finally, we use a `mutate()` call to create a variable derived from other variables. This will create the $$\hat{p} = X/n$$ column. 

XX formatting is weird, could use single $ for Latex instead
...
And we then have the table with the percentages of people who are users over the total number of people of the same sex!
...
The `margins = TRUE` option makes sure that `tally()` ouputs the convenient Total columns just like in page 527!
...
```{r Vaccine4}
# XX shade = TRUE
vcd::mosaic(Required ~ Party, data = Vaccine)
```
...
To understand what is going on in this code, break it down into its components. We are creating a variable called `chiSqVaccine` and we are assigning the output of the `chisq.test()` call. The object stored in our variable will contain several useful fields as we will see. The first one is the expected values. To extract it from the object we use the `with()` function. 
...
All this useful features are already built into how R's `chisq.test()` function works. 

Note: There is an error in the $$\chi^2$$ value in the book. While it showed the correct machine output, it specified the wrong $$\chi^2$$ squared value. XX formatting is weird with Latex again. Single $?
...
And finally our $$\chi^2$$ statistic. XX latex formatting

```{r Health7}
chiSqHealth
```

Remember these are all possible thans to the functionality of the `chisq.test()` function.
...
We will use the same `chisq.test()` function from before. It is important to note the new behavior we expect from the function when we provide a vector of the population proportions. The expected counts and the $$\chi^2$$ value will depend on this new parameter. XX latex formatting

```{r ACT4}
chisqACT <- chisq.test(tally(~ State, data = ACT), p = c(0.105, .172, .164, .188, .07, .301), 
                       correct = FALSE)
```
...
Another example of a field included in the return value of the `chisq.test()` is the `residuals` field. Let's take a look. 

...
We will use the same `chisq.test()` function from before. It is important to note the new behavior we expect from the function when we provide a vector of the population proportions. The expected counts and the $$\chi^2$$ value will depend on this new parameter. 

```{r}
chisqACT <- chisq.test(tally(~ State, data = ACT), p = c(0.105, .172, .164, .188, .07, .301), correct = FALSE)
```

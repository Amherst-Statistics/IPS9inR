...
digits <- 1:9
data <- data.frame(Outcome = benlaw(digits))
#XX ERROR HERE, NOT ABLE TO KNIT
gf_bar(. ~ , data = data)
baseBarplot <- barplot(benlaw(digits), names.arg = digits, xlab = "Outcomes", 
                       ylim = c(0, .40))
```
...
Figure 4.7 (page 239): 
```{r}
trials <- data.frame(Heads = rbinom(2000, 4, 0.5))
gf_histogram(~ Heads, data = trials)
```

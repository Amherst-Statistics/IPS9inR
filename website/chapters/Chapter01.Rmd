...
gf_histogram(~ ., data = Online_data) # XX data = df parameter
```

XX personal note: after working out the histogram, address this
R orders the x-axis alphabetically, placing "Other" before "Wikipedia". However, we can make
slight modifications by nesting the `desc()` within the `arrange()` function. The output now matches the graph on page 10 and graphs

...
XX There is a missing link in the next sentence
...
Online_data <- c(rep("Google", 406), rep("Library", 75), rep("Wikipedia", 52), rep("Other", 19))
...
XX In the previous sentence Total should not be capitalized
...
XX The pipe operator has not been demonstrated so far (the next sentence says it has)
...
gf_histogram(~ Online_data, Online_data, stat = "count") # XX Should we be used to specifying the data = df parameter)
```
...
# XX This statement needs a comment that explains why the object is being modified in two steps
...
labels <- c("75 <= IQ & IQ < 85", "85 <= IQ & IQ < 95", 
  "95 <= IQ & IQ < 105", "105 <= IQ & IQ < 115", "115 <= IQ & IQ < 125", 
  "125 <= IQ & IQ < 135", "135 <= IQ & IQ < 145", "145 <= IQ & IQ < 155")
labels <- as.factor(seq(from = 80, to = 150, by = 10))
IQ_Count <- IQ %>% 
  mutate(Class = cut(IQ, levels, labels = labels)) 
gf_histogram(~ Class, data = IQ_Count, stat = "count") # XX Consisder gf_bar?
```
...
XX Is there suppose to be an example accompanying the paragraph above about mean and round? 

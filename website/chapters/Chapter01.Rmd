...
```
We have piped the `mean()` output into the `round()` function and specified the number of digits 
...
TITANIC <- read_csv("https://nhorton.people.amherst.edu/ips9/data/chapter01/EG01-36TITANIC.csv")
```

### Section 1.4: Density Curves and Normal Distributions

options(digits = 3)
```{r eg 1-46, message = FALSE}
IQ <- read_csv("https://nhorton.people.amherst.edu/ips9/data/chapter01/EG01-46IQ.csv")
```

```{r eg1-48, message = FALSE}
STUBHUB <- read_csv("https://nhorton.people.amherst.edu/ips9/data/chapter01/EG01-48STUBHUB.csv")
```

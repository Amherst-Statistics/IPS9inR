...
#Figure 16.3(a)
# XX BL please extirpate all uses of pull!
...
College_GPA <- College_GPA %>% 
  mutate(sex = dplyr::recode(sex, 
        ...
        "2" = "Female"), 
        sex = as.factor(sex))
# XX BL is the is.factor call necessary here?
## Figure 16.10(a)
...
```
XX BL: Not sure why the boot method call is not working (please check with MC)
...
diffMeanDRP <- function(Treatment, Control){
 Sample_Treatment <- resample(Treatment)
 Sample_Control<- resample(Control)
 diff(c(mean(~ drp, data = Sample_Treatment), mean(~ drp, data = Sample_Control)))
}
numsim <- 1000
DRP_sim <- do(numsim) * diffMeanDRP(sample(Treatment), Control)

## XX Not sure if I'm understanding the permutation test correctly
# XX BL shouldn't this be something like:
# do(numsim) * diffmean(Y ~ shuffle(X), data = DRP)?
```

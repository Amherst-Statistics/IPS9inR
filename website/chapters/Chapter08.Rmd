...
calculatePower <- function(altmean = x, n, alpha){
  range <- 1-alpha/2
  se <- sqrt(100/n)
  bound <- qnorm(range, mean = 0.6, se) #sig lvl in null dist
  1 - pnorm(bound, altmean, se) + pnorm(-bound, altmean, se) #calc area past sig lvl
} 

gf_function(fun = calculatePower, args = list(altmean = x, n = 97, alpha = 0.05), xlim = c(0.3, 0.9))
## XX NH error message: Computation failed in `stat_function()`: unused argument (x_trans); not sure how to specify the
## arguments otherwise
```
